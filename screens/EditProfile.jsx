...
export default function EditProfile() {
    ...
    try {
      ...
      if (response.message === "File uploaded!") {
        const updateProfile = await fetch(
          `${api}/updateUserProfile/${user.email}`
        );
 ...
 const pickImage = async () => {
  ...
  if (status !== "granted") {
    errorToast("Permission to access media library was denied.");
    return;
  }
  ...
  try {
    ...
    uploadImage(newPath);
    // console.log("✅ New image saved:", newPath);
  } catch (error) {
    console.log("Error picking image:", error);
    errorToast("An error occurred while picking an image.");
  }
};
    ...
    <View style={theme === "light" ? edit.main : darkEdit.main}>
      ...
      <ScrollView>
        <TouchableOpacity
          activeOpacity={0.8}
          onPress={() => pickImage()}
          style={theme === "light" ? edit.profile : darkEdit.profile}
        >
          {isUploading ? (
            <ActivityIndicator
              color={color.main}
              ...
              style={[
                edit.profilePicture,
                { position: "absolute", zIndex: 5, backgroundColor: "#0005" },
              ]}
            />
          ) : (
            ""
          ...
          <Image
            source={
              profilePicture ? { uri: profilePicture } : defaultProfilePicture
            }
            style={
              theme === "light" ? edit.profilePicture : darkEdit.profilePicture
            }
          />
          <Ionicons
            name="camera"
            color={"white"}
            style={theme === "light" ? edit.profileIcon : darkEdit.profileIcon}
          />
        </TouchableOpacity>

        <Text
          style={
            theme === "light" ? edit.sectionHeader : darkEdit.sectionHeader
          }
        >
          First Name
        </Text>
        ...
        <TouchableOpacity
          ...
          onPress={() => save()}
          style={
            theme === "light" ? edit.buttonContainer : darkEdit.buttonContainer
          }
        >
          {isSaving ? (
            ...
            <Text style={theme === "light" ? edit.button : darkEdit.button}>
              Save
            </Text>
          )}
        </TouchableOpacity>
