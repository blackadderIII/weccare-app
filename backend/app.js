...
app.post("/createUser", (req, res) => {
  ...
  const query =
    "INSERT INTO `carecard`.`users`(`firstname`,`lastname`,`profilepicture`,`email`,`password`,`department`,`designation`) VALUES (?, ?, ?, ?, ?, ?, ?);";
...
app.get("/getCardAmount/:email", (req, res) => {
  const email = req.params.email;
  const pendingquery =
    "SELECT COUNT(*) as pendingCards FROM cards WHERE status = 'Pending' AND observerEmail = ?;";
  const completedquery =
    "SELECT COUNT(*) as completedCards FROM cards WHERE status = 'Completed' AND observerEmail = ?;";
  const uncompletedquery =
    "SELECT COUNT(*) as uncompletedCards FROM cards WHERE status = 'Uncompleted' AND observerEmail = ?";
...
app.get("/getPendingCards/:email", (req, res) => {
  const email = req.params.email;
  const pendingquery =
    "SELECT * FROM cards WHERE status = 'Pending' AND observerEmail = ?;";
...
app.get("/getCompletedCards/:email", (req, res) => {
  const email = req.params.email;
  const completedquery =
    "SELECT * FROM cards WHERE status = 'Completed' AND observerEmail = ?;";
...
app.get("/getUncompletedCards/:email", (req, res) => {
  const email = req.params.email;
  const uncompletedquery =
    "SELECT * FROM cards WHERE status = 'Uncompleted' AND observerEmail = ?;";
...
app.get("/getRecentCards/:email", (req, res) => {
  ...
  const query =
    "SELECT * FROM cards WHERE observerEmail = ? ORDER BY dateAdded DESC LIMIT 3";
